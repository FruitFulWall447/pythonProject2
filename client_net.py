...
class ClientNet:
    ...
    def send_large_udp_data(self, data, data_type, shape_of_frame=None):
        if len(data) > int(self.mtu * 0.5):
            sliced_data = slice_up_data(data, int(self.mtu * 0.5))
        else:
            sliced_data = [data]
    ...
    def ask_for_song_by_title(self, title):
        try:
            message = {"message_type": "playlist_song_bytes_by_title", "title": title}
            self.send_message_dict_tcp(message)
        except socket.error as e:
            print(e)
    ...
    def ask_for_code_to_change_password_for_logged_user(self):
        try:
            message = {"message_type": "change_settings", "change_value": "password"
                       }
            self.send_message_dict_tcp(message)
        except socket.error as e:
            print(e)
