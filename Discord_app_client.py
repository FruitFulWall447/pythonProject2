...
class SplashScreen(QWidget):
    ...
    def update_progress_bar_value(self, added_value):
        ...
        if new_value < 100:
            ...
            self.set_value_as_animation(100)

    def set_value_as_animation(self, new_value):
        original_value = self.progressBar.value()
        for i in range(original_value, new_value+1):
            time.sleep(0.01)
...
class MainPage(QWidget):  # main page doesnt know when chat is changed...
    ...
    def right_click_object_func(self, pos, parent, button, actions_list, chat_name=None, group_id=None):
        try:
            ...
            if chat_name is not None:
                ...
                group_name = self.get_group_name_by_id(group_id)
                full_group_name = f"({group_id}){group_name}"
                action_text = f"call {group_name}"
                action = menu.addAction(action_text)
    ...
    def scroll_back_to_index_before_update(self, n_last_widgets):
        self.messages_content_saver.scroll_down_by_n_widgets(n_last_widgets)
    ...
    def insert_message_that_client_send(self, message_dict):
        ...
        self.messages_content_saver.call_scroll_maximum_with_delay()
